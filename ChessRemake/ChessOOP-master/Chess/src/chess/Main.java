...
public class Main extends JFrame implements MouseListener
{
	...
	public Main()
    {
		...
		//=============================================================================================
		//Code added/changed by Cameron Sadusky
		askForGameMode();
	...
	//=============================================================================================
	//code added by Cameron Sadusky
	public void askForGameMode() {
	    String[] options = {"Normal Chess", "Chess960"};
	    int choice = JOptionPane.showOptionDialog(this, 
	            "Select the type of chess game", 
	            "Game Mode", 
	    ...
	    if (choice == 0) {
	        chess960 = false; // Normal chess
	    } else if (choice == 1) {
	        chess960 = true; // Chess960
	    }
	}
	
	
	//A function to randomize the back row when the user decides to play chess 960
	public void setChess960Board() {
		Random rand = new Random();
	    pieces.Piece P;
	    Cell cell;

	    ArrayList<Integer> backRowSpots = new ArrayList<>(Arrays.asList(0, 1, 2, 3, 4, 5, 6, 7));

	    // Possible bishop placements (opposite colors)
	    int[][] BishopPlacement = {
	        {0, 1}, {0, 3}, {0, 5}, {0, 7}, {1, 2}, {1, 4}, {1, 6}, {2, 3}, 
	        {2, 5}, {2, 7}, {3, 4}, {3, 6}, {4, 5}, {4, 7}, {5, 6}, {6, 7}
	    };

	    // Select a random bishop placement
	    int[] randomBishopPlacement = BishopPlacement[rand.nextInt(BishopPlacement.length)];
	    int bishopSpotOne = randomBishopPlacement[0];
	    int bishopSpotTwo = randomBishopPlacement[1];

	    // Remove bishops from back row spots
	    backRowSpots.remove(Integer.valueOf(bishopSpotOne));
	    backRowSpots.remove(Integer.valueOf(bishopSpotTwo));

	    // Select the king's position ensuring space for rooks
	    int kingIndex = rand.nextInt(1, backRowSpots.size() - 1); // Avoid first/last positions
	    int kingSpot = backRowSpots.get(kingIndex);

	    // Find two rook spots: one to the left and one to the right of the king
	    ArrayList<Integer> leftSide = new ArrayList<>();
	    ArrayList<Integer> rightSide = new ArrayList<>();

	    for (int spot : backRowSpots) {
	        if (spot < kingSpot) {
	            leftSide.add(spot);
	        } else if (spot > kingSpot) {
	            rightSide.add(spot);
	        }
	    }

	    if (leftSide.isEmpty() || rightSide.isEmpty()) {
	        throw new IllegalStateException("Failed to place rooks properly.");
	    }

	    int lowerRookSpot = leftSide.get(rand.nextInt(leftSide.size()));
	    int higherRookSpot = rightSide.get(rand.nextInt(rightSide.size()));

	    // Remove king and rooks from available spots
	    backRowSpots.remove(Integer.valueOf(kingSpot));
	    backRowSpots.remove(Integer.valueOf(lowerRookSpot));
	    backRowSpots.remove(Integer.valueOf(higherRookSpot));

	    // Place the queen
	    int queenSpot = backRowSpots.remove(rand.nextInt(backRowSpots.size()));

	    // Place knights in remaining two spots
	    int knightOne = backRowSpots.remove(0);
	    int knightTwo = backRowSpots.remove(backRowSpots.size() - 1);
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                ...
                // Define and place back row pieces (Chess960-specific placements)
                if (i == 0) {
                    // Black pieces (back row)
                    if (j == bishopSpotOne)
                        P = bb01; // Place first black bishop
                    else if (j == bishopSpotTwo)
                        P = bb02; // Place second black bishop
                    else if (j == kingSpot)
                        P = bk; // Place black king
                    else if (j == queenSpot)
                        P = bq; // Place black queen
                    else if (j == lowerRookSpot)
                        P = br01; // Place lower rook
                    else if (j == higherRookSpot)
                        P = br02; // Place higher rook
                    else if (j == knightOne)
                        P = bk01; // Place black knight 1
                    else if (j == knightTwo)
                        P = bk02; // Place black knight 2
                } else if (i == 7) {
                    // White pieces (back row)
                    if (j == bishopSpotOne)
                        P = wb01; // Place first white bishop
                    else if (j == bishopSpotTwo)
                        P = wb02; // Place second white bishop
                    else if (j == kingSpot)
                        P = wk; // Place white king
                    else if (j == queenSpot)
                        P = wq; // Place white queen
                    else if (j == lowerRookSpot)
                        P = wr01; // Place lower rook
                    else if (j == higherRookSpot)
                        P = wr02; // Place higher rook
                    else if (j == knightOne)
                        P = wk01; // Place white knight 1
                    else if (j == knightTwo)
                        P = wk02; // Place white knight 2
                } else if (i == 1) {
                    // Black pawns (second row)
                    P = bp[j];
                } else if (i == 6) {
                    // White pawns (second last row)
                    P = wp[j];
                }
                
                // Create a new cell with the piece P placed and add mouse listener
                cell = new Cell(i, j, P);
                cell.addMouseListener(this);
                board.add(cell);
                
                // Store the created cell in the boardState
                boardState[i][j] = cell;
            }
        }
